...
// ----------------------------------------------------------------------------------
  ...
  for (int i = 0; i < imax; i++) {
    for (int j = 0; j < jmax; j=j+vect) { //vect _must_ be a factor of jmax
      ...
      x[i*jmax + j+w] = (data_t) (j+i*jmax+w)+1;
      u[i*jmax + j+w] = (data_t) (j+i*jmax+w)+1;
      v[i*jmax + j+w] = (data_t) (j+i*jmax+w)+1;
      y[i*jmax + j+w] = (data_t) (j+i*jmax+w)+1;
...
// ---------------------------------------------------------------------------------- 
  ...
  printf("TY: Starting Host run\n");
    int dt,freq,f,pi,alpha,beta;
  
  //from TIR, int version
  //#define ALPHA -1
  //#define BETA  3
  //#define DT    432
  //#define ONE_MINUS_BETA -2
  //#define ONE_PLUS_BETA  4
  ...
  //!*********************************
    ...
    for (int i = 0; i < imax; i++) {
      for (int j = 0; j < jmax; j++) {
        //! velocity predictor
        //if (mode == 1) {
          un[i*jmax + j] = (u[i*jmax + j]*(10-beta)+alpha*v[i*jmax + j])/(1+beta);
          vn[i*jmax + j] = (v[i*jmax + j]*(10-beta)-alpha*u[i*jmax + j])/(1+beta);
        //}
        //else {
...
// ----------------------------------------------------------------------------------
   ...
   // Get Accelerator compute device
    ...
    for (uint i = 0; i < how_many_words_to_compare; i=i+vect) {
      ...
      //   ){
        printf("ERROR,i=%d:: xn(e,a)=(%d,%d); un(e,a)=(%d,%d); vn(e,a)=(%d,%d); yn(e,a)=(%d,%d); \n"
          , i+w
