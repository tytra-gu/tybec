   ...
   // Get Accelerator compute device
    ...
    for (uint i = 0; i < how_many_words_to_compare; i=i+vect) {
      ...
      if (
         ...
         (h_axi00_ptr0_output[i*NOUTPUTS + (1*vect) + w]!= un[r*jmax + c+w]) ||
         (h_axi00_ptr0_output[i*NOUTPUTS + (2*vect) + w]!= vn[r*jmax + c+w]) ||
        ...
        , un[r*jmax + c+w], h_axi00_ptr0_output[i*NOUTPUTS + (1*vect) + w]
        , vn[r*jmax + c+w], h_axi00_ptr0_output[i*NOUTPUTS + (2*vect) + w]
