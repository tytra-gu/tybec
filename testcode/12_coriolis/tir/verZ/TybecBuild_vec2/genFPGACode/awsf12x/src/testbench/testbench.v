...
wire [`STREAMW-1:0] x_stream_data_s0;
wire [`STREAMW-1:0] x_stream_data_s1;
wire [`STREAMW-1:0] u_stream_data_s0;
wire [`STREAMW-1:0] u_stream_data_s1;
wire [`STREAMW-1:0] v_stream_data_s0;
wire [`STREAMW-1:0] v_stream_data_s1;
wire [`STREAMW-1:0] y_stream_data_s0;
wire [`STREAMW-1:0] y_stream_data_s1;
wire [`STREAMW-1:0] xn_stream_data_s0;
wire [`STREAMW-1:0] xn_stream_data_s1;
wire [`STREAMW-1:0] vn_stream_data_s0;
wire [`STREAMW-1:0] vn_stream_data_s1;
wire [`STREAMW-1:0] un_stream_data_s0;
wire [`STREAMW-1:0] un_stream_data_s1;
wire [`STREAMW-1:0] yn_stream_data_s0;
wire [`STREAMW-1:0] yn_stream_data_s1;
...
//arrays in the dram
reg [`DATAW-1:0]  x  [0:`SIZE-1];
reg [`DATAW-1:0]  u  [0:`SIZE-1];
reg [`DATAW-1:0]  v  [0:`SIZE-1];
reg [`DATAW-1:0]  y  [0:`SIZE-1];
reg [`DATAW-1:0]  xn  [0:`SIZE-1];
reg [`DATAW-1:0]  vn  [0:`SIZE-1];
reg [`DATAW-1:0]  un  [0:`SIZE-1];
reg [`DATAW-1:0]  yn  [0:`SIZE-1];
...
$readmemh("/cygdrive/c/WAQAR/GU/Work/_TyTra_BackEnd_Compiler_/TyBEC/testcode/12_coriolis/tir/verZ/../../c/ver3/verifyChex.dat", resultfromC);
...
assign { xn_stream_data_s1 ,xn_stream_data_s0 ,vn_stream_data_s1 ,vn_stream_data_s0 ,un_stream_data_s1 ,un_stream_data_s0 ,yn_stream_data_s1 ,yn_stream_data_s0 } = packed_data_out;
...
(
   .aclk      (clk)
  ,.areset    (~rst_n)
  ,.s_tvalid  ({ivalid_todut, ivalid_todut, ivalid_todut, ivalid_todut})
  ,.s_tdata   (packed_data_in)
  ,.s_tready  ({iready_fromdut0, iready_fromdut1, iready_fromdut2, iready_fromdut3})
...
assign x_stream_data_s1 =  x[lincount+1] ;
assign u_stream_data_s0 =  u[lincount+0] ;
assign u_stream_data_s1 =  u[lincount+1] ;
assign v_stream_data_s0 =  v[lincount+0] ;
assign v_stream_data_s1 =  v[lincount+1] ;
assign y_stream_data_s0 =  y[lincount+0] ;
assign y_stream_data_s1 =  y[lincount+1] ;
...
always @(posedge clk) 
  if(ovalid_fromdut) begin 
    ...
    xn[effaddr+1] <= xn_stream_data_s1;
    vn[effaddr+0] <= vn_stream_data_s0;
    vn[effaddr+1] <= vn_stream_data_s1;
    un[effaddr+0] <= un_stream_data_s0;
    un[effaddr+1] <= un_stream_data_s1;
    yn[effaddr+0] <= yn_stream_data_s0;
    yn[effaddr+1] <= yn_stream_data_s1;
...
// -----------------------------------------------------------------------------
initial  begin
  $fdisplay(fverify, ":: Time stamp = Tue Aug 13 15:13:19 2019 ::\n\n");
