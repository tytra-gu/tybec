...
// ======================================================================
  ...
  for (int i=0;i<ntot;i++) {  
    dyn1(dt, dx, dy, g, eta_g, un_g, u_g, wet_g, v_g, vn_g, h_g, etan_g, BytesPerWord); 
    dyn2(dt, dx, dy, g, eta_g, un_g, vn_g, h_g, etan_g, BytesPerWord); 
    shapiro(wet_g, etan_g, eps_g, eta_g); //for shapiro, etan is input, eta is output
    updates  (h_g , hzero_g, eta_g, u_g, un_g, v_g, vn_g, wet_g, hmin_g);
  }
  
  // log results
  //-------------------------
  log_results(eta_g, h_g, u_g, v_g, wet_g);
...
//fdebug= fopen ("debug.dat","w");
  ...
  for (j=0; j<=ROWS-1; j++) {
    for (k=0; k<=COLS-1; k++) {
      u[j*COLS + k]   = (host_t) 3.14+j*COLS + k + 1;
      v[j*COLS + k]   = (host_t) 3.14+j*COLS + k + 1;
      eta[j*COLS + k] = (host_t) 3.14+j*COLS + k + 1;
      h[j*COLS + k]   = (host_t) 3.14+j*COLS + k + 1;
      //wet[j*COLS + k] = (host_t) j*COLS + k + 1;
      wet[j*COLS + k] = 1;
      //outputs
      etan[j*COLS + k] = (host_t) 0;
      un[j*COLS + k]   = (host_t) 0;
      vn[j*COLS + k]   = (host_t) 0;
    }
  }
...
//-------------------------------------------
  //-dt removed just for testing TIR gen HDL 
  for (j=1; j<= ROWS-2; j++) {
    for (k=1; k<= COLS-2; k++) {
      du[j*COLS + k]  = -dt 
                      * g
...
//---------------------------------
  for (j=1; j<= ROWS-2; j++) {
    for (k=1; k<= COLS-2; k++) {
      ...
      un[j*COLS + k]  = 0.0;
      //un[j*COLS + k]  = 123;
      //printf("OUTSIDE:Calculating un for %d\n",j*COLS + k);
      uu = u[j*COLS + k];
      duu= du[j*COLS + k];
...
//sea level predictor
//--------------------
//abs has been defined to bo NOP, as I don't yet have an abs on TyBEC
  for (j=1; j<= ROWS-2; j++) {
    for (k=1; k<= COLS-2; k++) {   
      hep = 5*( un[j*COLS + k] + abs(un[j*COLS + k]) ) * h[j*COLS + k  ];
      hen = 5*( un[j*COLS + k] - abs(un[j*COLS + k]) ) * h[j*COLS + k+1];
      hue = hep+hen;

      hwp = 5*( un[j*COLS + k-1] + abs(un[j*COLS + k-1]) ) * h[j*COLS + k-1];
      hwn = 5*( un[j*COLS + k-1] - abs(un[j*COLS + k-1]) ) * h[j*COLS + k  ];
      huw = hwp+hwn;

      hnp = 5*( vn[j*COLS + k] + abs(vn[j*COLS + k]) ) * h[    j*COLS + k];
      hnn = 5*( vn[j*COLS + k] - abs(vn[j*COLS + k]) ) * h[(j+1)*COLS + k];
      hvn = hnp+hnn;

      hsp = 5*( vn[(j-1)*COLS + k] + abs(vn[(j-1)*COLS + k]) ) * h[(j-1)*COLS + k];
      hsn = 5*( vn[(j-1)*COLS + k] - abs(vn[(j-1)*COLS + k]) ) * h[    j*COLS + k];
      hvs = hsp+hsn;
...
void updates  ( host_t *h 
  ...
  for (int j=0; j<= ROWS-1; j++) {
    for (int k=0; k<=COLS-1; k++) {
      ...
      wet[j*COLS + k] = 1;
      if ( h[j*COLS + k] < hmin )
            wet[j*COLS + k] = 0; //WN: I don't currently have LT for FP in the tybec gen code, so need to change something for compatibility.
      //u, v updates
...
void log_results ( host_t* eta_g
  ...
  for (int i = 0; i < ROWS; ++i) {
    for (int j = 0; j < COLS; ++j) {
      fprintf(feta_g  ,"%f,  ", *(eta_g   + i*COLS + j));
      fprintf(fh_g    ,"%f,  ", *(h_g     + i*COLS + j));
      fprintf(fu_g    ,"%f,  ", *(u_g     + i*COLS + j));
      fprintf(fv_g    ,"%f,  ", *(v_g     + i*COLS + j));
      fprintf(fwet_g  ,"%f,  ", *(wet_g   + i*COLS + j));
    }//j
    fprintf(feta_g  ,"\n");
    fprintf(fh_g    ,"\n");
    fprintf(fu_g    ,"\n");
