...
//fdebug= fopen ("debug.dat","w");
  ...
  for (j=0; j<=ROWS-1; j++) {
    for (k=0; k<=COLS-1; k++) {
      u[j*COLS + k]   = (host_t) j*COLS + k + 1;
      v[j*COLS + k]   = (host_t) j*COLS + k + 1;
      eta[j*COLS + k] = (host_t) j*COLS + k + 1;
      h[j*COLS + k]   = (host_t) j*COLS + k + 1;
      //wet[j*COLS + k] = (host_t) j*COLS + k + 1;
      wet[j*COLS + k] = 1;
      //outputs
      etan[j*COLS + k] = (host_t) j*COLS + k + 1;
      un[j*COLS + k]   = (host_t) j*COLS + k + 1;
      vn[j*COLS + k]   = (host_t) j*COLS + k + 1;
    }
  }
...
void log_results ( host_t* eta_g
  ...
  for (int i = 0; i < ROWS; ++i) {
    for (int j = 0; j < COLS; ++j) {
      ...
      fprintf(fwet_g  ,"%d,  ", *(wet_g   + i*COLS + j));
    }//j
    fprintf(feta_g  ,"\n");
    fprintf(fh_g    ,"\n");
    fprintf(fu_g    ,"\n");
