...
int main(void) {
  ...
  vout = (data_t *)malloc(sizeof(data_t)*SIZE_REDUCED); //reduced output "stream"
  ...
  //kernel B reduces the stream size 
  vconn_B_to_C = (data_t *)malloc(sizeof(data_t)*SIZE_REDUCED); 
  vconn_C_to_D = (data_t *)malloc(sizeof(data_t)*SIZE_REDUCED);
...
void post ( FILE *fp
  ...
  //Although some streams, including output, are "reduced" to a scalar, I stil log for all indices for debugging
  //For such reduced streams, I just replicate the reduced value found at index 0
	//fprintf(fp, "-------------------------------------------------------------------------------------------------------------------------------------------\n");
	fprintf(fp, "		   i,	   vin0(i),	   vin1(i),		  vconn_A_to_B(i),	 vconn_B_to_C(0),	vconn_C_to_D(0),	vout(0)	   \n");
	//fprintf(fp, "-------------------------------------------------------------------------------------------------------------------------------------------\n");
	for (int i=0;i<SIZE;i++){ 
...
#endif	  
				 , i,  vin0[i],	 vin1[i], vconn_A_to_B[i],	vconn_B_to_C[0],   vconn_C_to_D[0],	 vout[0]);
