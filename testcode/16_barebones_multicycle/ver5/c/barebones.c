...
int main(void) {
  ...
  //internal variables
  data_t* vconn_A_to_B_C;
  data_t* vconn_B_to_D;
  data_t* vconn_C_to_D;
  vconn_A_to_B_C = (data_t *)malloc(sizeof(data_t)*SIZE);
  vconn_B_to_D = (data_t *)malloc(sizeof(data_t)*SIZE);
  ...
  //#pragma DEVICE_CODE_START
	  ...
	  kernel_B ( vconn_A_to_B_C
             , vconn_B_to_D
             );
					 
	  kernel_C ( vconn_A_to_B_C
             , vconn_C_to_D
             );
...
void post ( FILE *fp
	...
	fprintf(fp, "-------------------------------------------------------------------------------------------------------------------------------------------\n");
	fprintf(fp, "		   i,	   vin0(i),	   vin1(i),		  vcon`n_A_to_B_C(i),	 vconn_B_to_D(i),	vconn_C_to_D(i),	vout(i)	   \n");
	fprintf(fp, "-------------------------------------------------------------------------------------------------------------------------------------------\n");
	for (int i=0;i<SIZE;i++){ 
...
#endif	  
				 , i,  vin0[i],	 vin1[i], vconn_A_to_B_C[i],	vconn_B_to_D[i],   vconn_C_to_D[i],	 vout[i]);
...
void computeKernelFunction ( data_t vin0[]
   ...
   for (int i=0; i<SIZE; i++) {
    vout[i] =  4*(vin0[i] + vin1[i]) + 4*((vin0[i] + vin1[i])*(vin0[i] + vin1[i]));
   }//for
