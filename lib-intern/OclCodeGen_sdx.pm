...
sub genHdlWrapper {
  ...
  #if($ioVect==1){
    #start the countup counter (we make connections in BIG ENDIAN fashion)
    $iStreamCount = $numInputStreams-1; 
    $oStreamCount = $numOutputStreams-1;
...
sub genSdxTopRtl {
  ...
  my $iStreamCount_withpadding  = 2**log2($iStreamCount); #total inputs with padding  
  ...
  $genCode =~ s/<ninputs>/$iStreamCount_withpadding/g;
  $genCode =~ s/<globalVect>/$ioVect/g;
...
sub genHostCpp {
  ...
  #HARDWIRED init value
  if ($datat eq 'float32') {
    $init_val = "3.14+(j+i*jmax+w)+1";
  }
  else {
    $init_val = "(j+i*jmax+w)+1" ;
  }
  ...
  foreach my $key (keys %{$main::CODE{main}{allocaports}}){
    ...
    $hostArraysForHostRunDeclare .= "  data_t* $nameOfMem = (data_t*) malloc(size_in_bytes);\n";
  ...
  #my log2 is already defined with CEIL to round up 
  my $iStreamCount_withpadding  = 2**log2($iStreamCount); #total inputs with padding  
  my $iStreamCount_added        = $iStreamCount_withpadding-$iStreamCount; 

  #padding if needed
  foreach my $p($iStreamCount..$iStreamCount_withpadding-1) {
    $init_dev_input .= "      h_axi00_ptr0_input  [(j+i*jmax)*NINPUTS + ($p*vect) + w] = (data_t) 0;\n";
  }
  ...
  $genCode =~ s/<ninputs>/$iStreamCount+$iStreamCount_added/g;
  ...
  if($main::ihc) {
    ...
    $genCode =~ s/<hostRun4GoldenResults>/\/\/ XXXX ADD YOUR HOST GOLDEN RUN CODE HERE XXXX/;
  }
