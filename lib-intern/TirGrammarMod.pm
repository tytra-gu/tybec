...
#Since I do not have cost of all symbols calc yet, so I need to check if it exists first before setting
  ...
  else                              {$xlabel = "($lat, $lfi, $sd)";}
  
 #is this node a PNDMAP?
  if (exists $main::CODE{$func}{symbols}{$symbol}{pndmap}) {
    my $pndmap = $main::CODE{$func}{symbols}{$symbol}{pndmap};
    $xlabel .= " x ".$pndmap if ($pndmap > 1);
  }
      ...
      if($cond) {
        ...
        if ($predecsok == 1) {
          ...
          foreach (@predecs) {
            ...
            if ($psymbol ne $key) {
              #my starting delay is starting delay of previous node + its latency (FI has no effect as it stalls the entire pipeline so no sync buffers needed)
              #$sd_me  = $sd_p + ($lat_p-1) + ($afi_p * $fpo_p); # "-1" was here as the next expression always added 1 even as minimum afi is 1
              $sd_me  = $sd_p + ($lat_p);# + ($afi_p * $fpo_p);
...
#  }
  ...
  foreach my $ckey ( keys %{$hash->{symbols}} ) {
    ...
    my $redSize = -1;
    $redSize = $hash->{symbols}{$ckey}{reductionSize} 
      if defined ($hash->{symbols}{$ckey}{reductionSize});
    ...
    if ($redSize < 1) {
      ...
      foreach (@predecs) {
        ...
        #SD does not take require informaion of FI of previous as we do not want to infer buffers due to FI > 1
        #my $sd_due2_this = $sd_p + ($lat_p-1) + ($afi_p * $fpo_p); # "-1" was here as the next expression always added 1 even as minimum afi is 1
        my $sd_due2_this = $sd_p + ($lat_p);# + ($afi_p * $fpo_p);
        ...
        if ($buff_size_4this > 0) {
          ...
          #buf is named after the connection, and the offset (not source-dest), as the same offset output may be going to multiple inputs
          #my $bufkey = $pkey."_buff".$buff_offset_4this; 
          my $bufkey = $connection."_buff".$buff_offset_4this; 
          #my $bufkey = $pkey."_buff"; #TEMP renamed so that I dont have to recompile when lat changes. But this namign convention will FAIL if multiple buffers from the same stream are made. FIXME
          $bufkey =~ s/\%//g; 
          ...
          if($main::dfGraph->has_vertex($bufnode)) {
            ...
            #print YELLOW; print "Buffer $bufnode for $pnode in $func alread exists::$previousBuffSize, $buff_size_4this, $buff_size \n"; print RESET;
          }
          ...
          my $buffProduces = $connection.'_buff'.$buff_offset_4this;
          ...
          #since multiple consumers possible, so only push if not already there
          push @{$main::CODE{$func}{symbols}{$bufkey}{produces}}     , $buffProduces 
            if (!( $buffProduces ~~ @{$main::CODE{$func}{symbols}{$bufkey}{produces}}));
          push @{$main::CODE{$func}{symbols}{$bufkey}{tapsAtDelays}} , $buff_offset_4this+1
            if (!( ($buff_offset_4this+1) ~~ @{$main::CODE{$func}{symbols}{$bufkey}{tapsAtDelays}}));
          ...
          #BUT if consumer buffer has multiple consumers?
          for my $i (0 .. $#{$consArrRef}) {
            ...
            if($$consArrRef[$i] eq $connection) {
              #print YELLOW; print "connection = $connection; buffProduces = $buffProduces; ckey = $ckey\n"; print RESET;
...
sub globalScheduler {  
  ...
  #one in and one out node would be identified by this point
...
sub actSTREAMWRITE{
  ...
  #main has a parameter "sizeOutput" as output stream sizes may be different from 
  #input stream sizes. That should be updated here. This assumed that ALL output streams
  #have the same size
  $main::CODE{$func}{bufsizeOutput} = $main::CODE{$func}{symbols}{$mystream}{size};
...
# 
# ----------------------------------------------------------------------------
sub MD_FUNC_RESOURCES{
  my @item = @{shift(@_)};
  my %item = %{shift(@_)};
  my @arg  = @{shift(@_)};
  my $func = $arg[0];
  
  $main::CODE{$func}{metadata}{aluts} = $item{INTEGER1};
  $main::CODE{$func}{metadata}{regs}  = $item{INTEGER2};
  $main::CODE{$func}{metadata}{bram}  = $item{INTEGER3};
  $main::CODE{$func}{metadata}{dsps}  = $item{INTEGER4};
}
...
sub actOFFSET_STREAM{
  ...
  #the streami may not have maxPos and maxNeg defined 
  #(this would be the case when streami is an internal stream, not an argument)
  $main::CODE{$func}{symbols}{$streami}{maxPosOffset} = 0
    if (!(exists $main::CODE{$func}{symbols}{$streami}{maxPosOffset}));
  $main::CODE{$func}{symbols}{$streami}{maxNegOffset} = 0
    if (!(exists $main::CODE{$func}{symbols}{$streami}{maxNegOffset}));
  ...
  #Cost::costSmache($func, $smachenode, $main::ioVect);  
...
sub actFUNC_CALL_INSTR{
  ...
  #is there pndmap on this function call?
  $main::CODE {$caller} {symbols} {$calleeWC} {pndmap} = 1; #default
  my $pndmap = @{$item[2]}[0];
  $main::CODE {$caller} {symbols} {$calleeWC} {pndmap} = $pndmap if defined ($pndmap);
...
# ----------------------------------------------------------------------------
  ...
  $main::CODE{$arg[3]}{symbols}{$arg[0]}{reductionSize} = eval($item{EXPRESSION});
  $main::CODE{$arg[3]}{symbols}{$arg[0]}{reductionOp}   = 'yes';
  
  #find out which of oper1 or oper2 input operand is _also_ the reduction output
  $main::CODE{$arg[3]}{symbols}{$arg[0]}{inputOperReducOut} = 'oper1' if ($arg[0] eq $arg[1]);
  $main::CODE{$arg[3]}{symbols}{$arg[0]}{inputOperReducOut} = 'oper2' if ($arg[0] eq $arg[2]);
  
  #set parent fuction type to be a "folding" function
  $main::CODE{$arg[3]}{func_type} = 'fold';
...
# ----------------------------------------------------------------------------
  ...
  my %item = %{shift(@_)};
  my @arg  = @{shift(@_)};  
  $main::CODE{$arg[1]}{symbols}{$arg[0]}{fi_md} = eval($item{EXPRESSION});
}
...
## ----------------------------------------
  ...
  $myop = 'compare_'."$item{TYPE_COMPARE}"  if($instType eq 'compare');
  ...
  #if this is not a reduction operation (no METADATA to indicate it is), set these parameters to default
  if (!(defined $main::CODE{$func}{symbols}{$myoperD}{reductionOp})) {
    $main::CODE{$func}{symbols}{$myoperD}{reductionOp}  ='no';
    $main::CODE{$func}{symbols}{$myoperD}{reductionSize}=-1;
  }
