...
#  else {
  ...
  #    print "accumulating cost for $func, $key\n";
    ...
    #If I have metadata for resource estimates, that over-rides any calculations
    if (defined $hash->{metadata}) {
      $hash->{resource}{regs}  = $hash->{metadata}{regs} ;
      $hash->{resource}{dsps}  = $hash->{metadata}{dsps} ;
      $hash->{resource}{aluts} = $hash->{metadata}{aluts};      
      $hash->{resource}{bram}  = $hash->{metadata}{bram} ;      
    }
...
#----------------------------------------
  ...
  #$lat = $pndmap  if ($pndmap > 1);
  #$lat = ($pndmap + $lat) if ($pndmap > 1);
  $lat = ($lfi + $lat) if ($lfi > 1);
...
#-----------------------------------
  ...
  #-------------------------
  ## latency ##
  my $lat =  0; #autoindex does not have any latency. It is in-synch with input ports which too have 0 latency.
  my $afi = 1;  
  my $lfi = 1;   
  my $efi = 1;  
  my $fpo = 1; 
...
#-----------------------------------
  ...
  #-------------
  #latency for ints
  my $lat =  1; #default latency for all integer primitive instructions 
  #$lat = 7 if ($op eq 'pow');
    # int pow is just being tested with stub function
  ...
  if ($dcat eq 'float') {
    ...
    #if (($op eq 'pow'))   {$lat =  27;} #this is the real latency from flopoco
    if (($op eq 'pow'))   {$lat =  1;} #Set to 1, I am using pow as a proxy for a stub multi-cycle iterative unit
    #if (($op eq 'pow'))   {$lat =  2;} #this is stub version for testing only
  ...
  # some iterative units have LFI  > 1 
  ...
  if($iterative) {
    #the only stalling instruction I have for now is "pow", with a default FI
    $lfi = 7 if ($op eq 'pow');
    
    #see if I had defined the FI via metadata though
    $lfi = $hash->{fi_md} if(defined($hash->{fi_md}));
  ...
  # FPO
  #----------
  #Firings per output are by default 1, unless this is a reduction operation
  my $fpo = 1; 
  if (defined $hash->{reductionOp}) {
    if ($hash->{reductionSize} > 1) {
      $fpo = $hash->{reductionSize};
    }
  }
...
#-----------------------------------
  ...
  my $lat = ($maxPosOff/$vect) + 1; 
    ...
    #see docs/stencilbuffers_withvectorization_calculations.xlsx for calculation for vectors
    #This calculation presumes that stencil-buffers are only ever inferred in the top level kernel
  my $afi = -1;  
  my $lfi = 1;   
  my $efi = -1;  
  my $fpo = 1; 
