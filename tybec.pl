...
#should I include code in the OCX host code from a presumed to be availaable "hostrun.c"?
our $ihc = '';
...
sub init{  
  ...
  GetOptions (
    ...
    , 'ihc'    => \$ihc             #Should I add host golden run code into OCX host file
    );
  ...
  #"t09"; #used for Hindawi, with SDX v2018.2, coalesced inputs, and coalesced outputs (old hdl-shell template)
  #"t10"; #used for Hindawi, with SDX v2018.2, coalesced inputs, and coalesced outputs (new hdl-shell template)
  $ocxTempVer= 't10'  if($coalIOs == 1);  
  ...
  if ($VERBOSE_DFG_ANNOT == 1){
    ...
    $text = "3 attributes: (latency AND local-firing-interval a.k.a clocks-per-output, AND Starting Delay))
    That is: (LAT, LFI, SD)\n\n";
  }
...
sub parse {
  ...
  #if you want to see what exactly is captured from the lines after removing comments
  my $tfh;
  my $tempfile = "tempfile.tirl";
  open($tfh, '>', $tempfile);
  print $tfh join ("", @lines);
  close($tfh);
...
sub generateHDL {
  ...
  #top Kernel is (can be) treated differently for vectorization 
  my $topKernel = $CODE{main}{topKernelName};
  $topKernel =~ s/\_\d+$//;
  ...
  # I was earlier vectorizing the main module, but now I am going one level deeper in order to integrate vectorization with stencils
  my $vect4main  = 1;
  
  #
  my $vect = 1; #vectorization is 1 for all module in general
  my $top; #top module or not?
  ...
  foreach (@nodes_conn) { 
    ...
    if($dfgroup eq 'main') {
      ...
      HdlCodeGen::genMapNode_hier($hdlfh, $module, 'untitled', $outputRTLDir, 'main', $CODE{main}, $vect4main);    
    }
    ...
    #print("::cat    = $cat\n");
      ...
      #print RED; print "::generate:: generating node = $_, parentFunc = $parentFunc, symbol = $symbol, ident = $ident, cat = $cat \n"; print RESET;
        #special case: create mutli-cycle stubs for iterative leaf nodes
        if ($CODE{$parentFunc}{symbols}{$symbol}{synthunit} eq 'pow') {
          HdlCodeGen::genMapNode_leaf_multicyclestub($hdlfh, $module, 'untitled', $outputRTLDir, $CODE{$parentFunc}{symbols}{$symbol}, 1);    
        } else {
          HdlCodeGen::genMapNode_leaf($hdlfh, $module, 'untitled', $outputRTLDir, $CODE{$parentFunc}{symbols}{$symbol}, 1);    
        }
      }
      ...
      if ($cat eq 'fifobuffer') {
        ...
        open(my $hdlfh, '>', $hdlFileName)
          or die "Could not open file '$hdlFileName' $!";
        HdlCodeGen::genAxiStreamBuffer($hdlfh, $module, 'untitled', $outputRTLDir, $CODE{$parentFunc}{symbols}{$symbol}, 1);    
      }
      ...
      if ($cat eq 'smache') {
        #if this stencil buffer is at the top hierarchy, then vectorize it (if we have global vectorization)
        $vect = $ioVect if($parentFunc eq $topKernel);
        ($ident =  $symbol) =~ s/(%|@)//; #remove %/@
        my $module    = $parentFunc."_".$ident;
        my $hdlFileName = "$outputRTLDir/$module".".v";
        open(my $hdlfh, '>', $hdlFileName)
      ...
      #-------------------------------
      if ($cat eq 'autoindex') {
        #if this auto-index is at the top hierarchy, then vectorize it (if we have global vectorization)
        $vect = $ioVect if($parentFunc eq $topKernel);
      ...
      #function-call (hierarchical) nodes
      #-----------------------------------
      #defaults
      $vect = 1; #no vectorization
      $top  = 0; #I am not top
      ...
      if($cat eq 'funcall') {
        ...
        #vectorization; if top Kernel, then we vectorize it. All others stay scalar
        if ($ident eq $topKernel) {
          $vect = $ioVect;
          $top  = 1;
        }
        ...
        open(my $hdlfh, '>', $hdlFileName)
          or die "Could not open file '$hdlFileName' $!";
        HdlCodeGen::genMapNode_hier($hdlfh, $module, 'untitled', $outputRTLDir, $dfgroup_local, $CODE{$ident}, $vect, $top);
        
        #does this function require a PNDMAP wrapper?
        my $pndmap = $CODE{$parentFunc}{symbols}{$symbol}{pndmap};
        if($pndmap > 1) {
          my $module    = $parentFunc."_".$ident."_pndmapWrapper";
          my $hdlFileName = "$outputRTLDir/$module".".v";
          open(my $hdlfh, '>', $hdlFileName)
            or die "Could not open file '$hdlFileName' $!";
          HdlCodeGen::genPndmapWrapper($hdlfh, $module, 'untitled', $outputRTLDir, $dfgroup_local, $CODE{$ident}, $vect, $pndmap);
        }
      }
  ...
  $hdlFileName = "$outputRTLDir/$module".".sv" if ($targetNode eq "awsf12x"); #sdx requires SystemVerilog
...
sub generateOCL {
  ...
  # ====================================================
    ...
    $err =copy("$srcdir/src/hdl/krnl_vadd_rtl_example_axi_read_master.sv"  
            , "$outputOCLDir/src/hdl/krnl_vadd_rtl_example_axi_read_master.sv");
    $err =copy("$srcdir/src/hdl/krnl_vadd_rtl_example_axi_write_master.sv" 
            , "$outputOCLDir/src/hdl/krnl_vadd_rtl_example_axi_write_master.sv");
    $err =copy("$srcdir/src/hdl/krnl_vadd_rtl_control_s_axi.v"     
            , "$outputOCLDir/src/hdl/krnl_vadd_rtl_control_s_axi.v");
    $err =copy("$srcdir/src/hdl/krnl_vadd_rtl_example_counter.sv"          
            , "$outputOCLDir/src/hdl/krnl_vadd_rtl_example_counter.sv");
...
sub printHelp {
  ...
  print "--ihc  [OFF]           :Should I add host golden run code (./hostrun.c) into OCX host file\n";
}
