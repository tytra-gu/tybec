...
int main(int argc, char** argv)
{
   ...
   // Get Accelerator compute device
    ...
    d_axi00_ptr0 = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX,  sizeof(int) * number_of_words, &d_bank_ext[0], NULL);
    ...
    err = 0;
    cl_uint d_size_in_bytes = 0;
    err |= clSetKernelArg(kernel, 0, sizeof(cl_uint), &d_size_in_bytes); // Not used in example RTL logic.
    err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_axi00_ptr0); 
    err |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &d_axi00_ptr0); // Not used in example RTL logic.
