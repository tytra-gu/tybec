...
(
   .aclk      (clk)
  ,.areset    (~rst_n)
  //,.s_tvalid  ({<ivalid_toduts>}) //since inputs coalesced into a single data signal, no need to do this now
  ,.s_tvalid  (ivalid_todut)
  ,.s_tdata   (packed_data_in)
  //,.s_tready  ({<iready_fromduts>}) //since outputs coalesced into a single data signal, no need to do this now
